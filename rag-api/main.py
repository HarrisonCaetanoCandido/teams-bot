...
from models.users import ParseCommandRequest, UserModel, AddressModel, GeoModel, CompanyModel
...
async def parseCommand(request: ParseCommandRequest):
    print(f"📨 Mensagem recebida: '{request.message}' do usuário: {request.user_id}")

    # simulacao de predicao
    rag_response = {
        "intent": "get-task-status",  # Intenção detectada
        ...
        "execute_flow": {
            ...
            "initiatedBy": "user-123",      # ID do usuário que solicitou
        ...
        try:
            filter_strings = UserModel(
                ...
                address=AddressModel(
                    street=rag_response["parameters"].get("address", {}).get("street", "all"),
                    suite=rag_response["parameters"].get("address", {}).get("suite", "all"),
                    city=rag_response["parameters"].get("address", {}).get("city", "all"),
                    zipcode=rag_response["parameters"].get("address", {}).get("zipcode", "all"),
                    geo=GeoModel(
                        lat=rag_response["parameters"].get("address", {}).get("geo", {}).get("lat", "all"),
                        lng=rag_response["parameters"].get("address", {}).get("geo", {}).get("lng", "all")
                    )
                ),
                phone=rag_response["parameters"].get("phone", "all"),
                website=rag_response["parameters"].get("website", "all"),
                company=CompanyModel(
                    name=rag_response["parameters"].get("company", {}).get("name", "all"),
                    catchPhrase=rag_response["parameters"].get("company", {}).get("catchPhrase", "all"),
            ...
            if whook_response.status_code == 200:
                ...
                if 'data' in whook_data:
                    filtered_users = treat_list_users(whook_data['data'], filter_strings)
    ...
    # -------------------------- GET TASK STATUS --------------------------
        ...
        try:
            ...
            whook_response = requests.get(f'{webhook_api_url}/api/commands/get-task-status')
            print('Webhook api status code response: ', whook_response.status_code)
            ...
            if whook_response.status_code == 200:
                ...
                if 'data' in whook_data:
                    filtered_tasks = treat_get_task_status(whook_data['data'], filter_strings)
    ...
    # -------------------------- EXECUTE FLOW --------------------------
        ...
        try:
            whook_response = requests.post(f'{webhook_api_url}/api/commands/execute-flow')
            print('Webhook api status code response: ', whook_response.status_code)
    ...
    return {
        ...
        "message": "Comando processado com sucesso" if whook_response and whook_response.status_code == 200 else "Erro no processamento do comando"
    }
