...
def treat_get_task_status(data: List[Dict[str, Any]], filter_strings: Tasks) -> List[Dict[str, Any]]:
    if not data:
        return data
    ...
    if filter_strings.userId != 'all':
        filtered_data = [task for task in filtered_data
    ...
    if filter_strings.completed != 'all':
        filtered_data = [task for task in filtered_data
                         if str(task.get('completed', '')).lower() == filter_strings.completed.lower()]
