...
import { adapter } from "../config/botAdapterTeams.config.js";
    ...
    await adapter.processActivity(req, res, async (context) => {
      ...
      if (context.activity.type === "message") {
        ...
        const data = {
          ...
          conversation_id: context.activity.conversation.id.toString(),
          service_url: context.activity.serviceUrl,
        };

        const response = await axios.post(
          `${RAGAPIURL}/ai/parse-command`,
          data,
          header
        );
        ...
        if (response.status == 200)
          await context.sendActivity(
            `${JSON.stringify(response.data.response.message)}`
          );
        else {
          console.error(`Status code ${response.status}`);
          await context.sendActivity(
            "Could not get a response from rag system."
          );
        }
      }
    });
