...
import { jobDefinition } from '../types/jobDefinition.types';
import { Job } from 'bullmq';

console.log('Flow worker started');

const worker = new Worker('flow-jobs', async (job: Job<jobDefinition>) => {
...
worker.on('failed', (job: Job<jobDefinition> | undefined, err: Error, prev: string) => {
    if (job) {
